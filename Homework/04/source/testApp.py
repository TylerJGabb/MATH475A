...
app.addNamedCheckBox('Symetric','chkbx_symetric',6,0,5)
...
def get_x_vec():
    ...
    makeOptimizedPlot(vals)


def update_x_label(scale_name):
    ...
    if app.getCheckBox('chkbx_symetric'):
        sym_dict = {'x1':'x6','x2':'x5','x3':'x4'}
        if scale_name not in sym_dict: return
        pos = app.getScale(scale_name)
        other = sym_dict[scale_name]
        app.setScale(other,-pos)
        update_x_label(other)
